...
const ProfileView: React.FC<ProfileViewProps> = ({ onClose, onReport, userId, onLogout }) => {
  ...
  const fetchContent = useCallback(async (type: 'all', signal: AbortSignal, pageNum: number) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Remove shorts from profile view
        ...
        if (type === 'all') {
            ...
            lastFetchedPostsPage.current = pageNum; 
  ...
  }, [isMe, targetId, mapApiPostToUI, cleanUrl, posts, deletedItemsIds]);
  ...
  const requestDeletePost = (id: string, isShort?: boolean) => {
    if (isMe) {
      // Shorts handling removed, only standard posts
  ...
  const handleTabChange = (tab: 'posts') => {
